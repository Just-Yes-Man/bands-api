...
} = require("../db/repositories/client-auth-repository");
const {
  AuthAuditRepository,
} = require("../db/repositories/auth-audit-repository");
const { LoginRateLimitStore } = require("../auth/login-rate-limit-store");
const { ClientJwtService } = require("../auth/client-jwt-service");
const {
  ClientAuthService,
} = require("../../application/services/client-auth-service");
const {
  RegisterClientUseCase,
} = require("../../application/use-cases/register-client-use-case");
const {
  LoginClientUseCase,
} = require("../../application/use-cases/login-client-use-case");
const { OrdersRepository } = require("../db/repositories/orders.repository");
const {
  OrderLinesRepository,
} = require("../db/repositories/order-lines.repository");
const {
  OrderStateEventsRepository,
} = require("../db/repositories/order-state-events.repository");
const {
  OrderRealtimeService,
} = require("../../application/services/order-realtime.service");
...
const {
  ListOrdersUseCase,
} = require("../../application/use-cases/list-orders.use-case");
const {
  GetOrderDetailUseCase,
} = require("../../application/use-cases/get-order-detail.use-case");
const {
  UpdateOrderLineProgressUseCase,
} = require("../../application/use-cases/update-order-line-progress.use-case");
const {
  CancelOrderLineUseCase,
} = require("../../application/use-cases/cancel-order-line.use-case");
const {
  CancelOrderUseCase,
} = require("../../application/use-cases/cancel-order.use-case");
const {
  MeasurementProcessesRepository,
} = require("../db/repositories/measurement-processes.repository");
const {
  MeasurementProcessStateHistoryRepository,
} = require("../db/repositories/measurement-process-state-history.repository");
const {
  MeasurementsRepository,
} = require("../db/repositories/measurements.repository");
const {
  MeasurementRealtimeService,
} = require("../../application/services/measurement-realtime.service");
const {
  MeasurementProcessesService,
} = require("../../application/services/measurement-processes.service");
const {
  MeasurementCaptureService,
} = require("../../application/services/measurement-capture.service");
const {
  CreateMeasurementProcessUseCase,
} = require("../../application/use-cases/create-measurement-process.use-case");
const {
  TransitionMeasurementProcessStateUseCase,
} = require("../../application/use-cases/transition-measurement-process-state.use-case");
const {
  RegisterMeasurementUseCase,
} = require("../../application/use-cases/register-measurement.use-case");
const {
  ListMeasurementProcessesUseCase,
} = require("../../application/use-cases/list-measurement-processes.use-case");
const {
  GetMeasurementProcessDetailUseCase,
} = require("../../application/use-cases/get-measurement-process-detail.use-case");
...
const buildContainer = () => {
  ...
  return {
    ...
    updateOrderLineProgressUseCase: new UpdateOrderLineProgressUseCase({
      ordersService,
    }),
    cancelOrderLineUseCase: new CancelOrderLineUseCase({ ordersService }),
    cancelOrderUseCase: new CancelOrderUseCase({ ordersService }),
    measurementProcessesService,
    measurementCaptureService,
    createMeasurementProcessUseCase: new CreateMeasurementProcessUseCase({
      measurementProcessesService,
    }),
    transitionMeasurementProcessStateUseCase:
      new TransitionMeasurementProcessStateUseCase({
        measurementProcessesService,
      }),
    registerMeasurementUseCase: new RegisterMeasurementUseCase({
      measurementCaptureService,
    }),
    listMeasurementProcessesUseCase: new ListMeasurementProcessesUseCase({
      measurementProcessesService,
    }),
    getMeasurementProcessDetailUseCase: new GetMeasurementProcessDetailUseCase({
      measurementProcessesService,
    }),
  };
};
