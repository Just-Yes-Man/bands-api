...
class OrdersService {
  ...
  async createOrder({ clienteId, lineas, actor, io }) {
    ...
    const createdLines = await this.orderLinesRepository.createMany({ orderId: order.id, lineas });
    ...
    await this.orderRealtimeService.emitCritical(io, 'order.status.changed.v1', {
      orderId: order.id,
      previousStatus: null,
  ...
  async updateLineProgress({ orderId, lineId, deltaProcesadas, deltaRechazadas, version, actor, io }) {
    const order = await this.ordersRepository.getById(orderId);
    if (!order) {
      throw asOrderError('ORDER_NOT_FOUND');
    }
    ...
    await this.orderRealtimeService.emitCritical(io, 'order.progress.updated.v1', {
      orderId,
      lineId,
      deltaProcesadas,
      deltaRechazadas,
  ...
  async recomputeAndPersistStatus({ orderId, io }) {
    ...
    const lineas = await this.orderLinesRepository.listByOrder(orderId);
    const nextStatus = computeOrderStatus({ currentStatus: current.estado, lineas });

    if (nextStatus !== current.estado) {
      const updated = await this.ordersRepository.transitionStatus({ orderId, status: nextStatus });
      await this.orderStateEventsRepository.record({
        orderId,
  ...
  buildProgress(lineas) {
    ...
    const totalSolicitadas = lineas.reduce((acc, l) => acc + Number(l.cantidad || 0), 0);
    const totalProcesadas = lineas.reduce((acc, l) => acc + Number(l.procesadas || 0), 0);
    const totalRechazadas = lineas.reduce((acc, l) => acc + Number(l.rechazadas || 0), 0);
    const totalCanceladas = lineas
      .filter((l) => l.estado_linea === 'CANCELADA')
      .reduce((acc, l) => acc + Math.max(0, Number(l.cantidad || 0) - Number(l.procesadas || 0) - Number(l.rechazadas || 0)), 0);

    const totalRestantes = Math.max(0, totalSolicitadas - totalProcesadas - totalRechazadas - totalCanceladas);
    const porcentajeAvance = totalSolicitadas > 0
      ? Number((((totalProcesadas + totalRechazadas + totalCanceladas) / totalSolicitadas) * 100).toFixed(2))
      : 0;
